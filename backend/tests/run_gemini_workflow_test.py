...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a one-page Gemini workflow test: choose sections, generate narration, "
            "select duplicate-safe highlight word instances, resolve bboxes, and map "
            "narration words to highlight points."
        )
    )
    parser.add_argument("pdf_path", help="Path to source PDF.")
    parser.add_argument("--page", type=int, default=1, help="1-based page to inspect.")
    parser.add_argument(
        "--max-sections",
        type=int,
        default=2,
        help="Maximum number of page sections Gemini may process.",
    )
    parser.add_argument(
        "--max-highlights",
        type=int,
        default=4,
        help="Maximum highlight word instances per processed section.",
    )
    parser.add_argument(
        "--max-candidates",
        type=int,
        default=180,
        help="Maximum word-instance candidates sent to Gemini per section.",
    )
    parser.add_argument(
        "--output-dir",
...
def _find_narration_word(
    *,
    ...
    same_word = [
        word for word in narration_words if word["normalized_word"] == normalized
    ]
    if same_word:
    ...
    fallback_words = [
        word for word in narration_words if word["normalized_word"] == fallback
    ]
    if fallback_words:
        ...
        highlight_bboxes.append(
            {
                ...
                "word": candidate["word"],
                "occurrence": candidate["pdf_occurrence"],
                "bbox": ref.bbox.model_dump(mode="json"),
        ...
        narration_links.append(
            {
                ...
                "normalized_narration_word": narration_word["normalized_word"]
                if narration_word
                else None,
                "narration_occurrence": narration_word["occurrence"]
                if narration_word
                else None,
            }
        )
    ...
    *,
    ...
    for section in sections:
        for bbox in section.get("highlight_bboxes", []):
            ...
            label = f"{bbox['word']}#{bbox['occurrence']}"
            draw.rectangle((x0, y0, x1, y1), outline=color, width=4)
            draw.text((x0, max(0, y0 - 22)), label, fill=color, font=font)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    image.save(output_path)


def _write_summary(output_path: Path, payload: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Page image: `{payload['page_image_path']}`",
        f"- Overlay image: `{payload.get('overlay_image_path')}`",
        "",
        "## Processed Sections",
    ]
    for section in payload["processed_sections"]:
        lines.extend(
            [
                "",
                f"### {section['section_id']} - {section['section_title']}",
                f"- Decision: `{section['decision']['use_section']}` - {section['decision']['reason']}",
                f"- Narration: {section['narration_text']}",
                f"- Highlight count: `{len(section['highlight_words'])}`",
                f"- Unresolved: `{len(section['unresolved'])}`",
            ]
        )
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def run_gemini_workflow_test(
    *,
    pdf_path: Path,
    page: int,
    max_sections: int,
    ...
    if not api_key:
        raise SystemExit("GOOGLE_API_KEY or GEMINI_API_KEY is required for this test.")
    ...
    truncated_pdf, cleanup = truncate_pdf(pdf_path, page)
    try:
        pages = render_pages(truncated_pdf, output_dir / "pages", settings.default_page_image_dpi)
        page_meta = next((page_meta for page_meta in pages if page_meta.page == page), None)
        if page_meta is None:
            raise SystemExit(f"Selected page {page} is unavailable.")
        ...
        if not candidate_sections:
            raise SystemExit(f"No Docling sections found on page {page}.")
        ...
        for section in candidate_sections:
            ...
            rows, lookup, grouped = _candidate_rows(
                section,
                word_index,
                page,
            ...
            accepted_count = sum(
                1 for item in processed_sections if item["decision"]["use_section"]
            )
        ...
        _draw_overlay(
            ...
            sections=[
                section for section in processed_sections if section["decision"]["use_section"]
            ],
        )
        ...
        summary_path = output_dir / f"gemini_workflow_page-{page:04d}.md"
        ...
        return {
            ...
            "overlay_path": payload["overlay_image_path"],
            "candidate_section_count": len(candidate_section_summaries),
            "processed_section_count": len(accepted),
            "highlight_count": sum(len(section["highlight_words"]) for section in accepted),
            "unresolved_count": sum(len(section["unresolved"]) for section in accepted),
            "payload": payload,
        }
    finally:
        if cleanup:
